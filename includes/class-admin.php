...
final class Admin {
	...
	public function render_page(): void {
		...
		if (isset($_GET['purged'])) {
			...
			echo esc_html(sprintf('Удалено записей лога: %d.', $count));
			echo '</p></div>';
		}
		...
		printf(
			'<a href="%s" class="nav-tab %s">Настройки</a>',
			esc_url(add_query_arg('tab', 'settings', $base)),
			$tab === 'settings' ? 'nav-tab-active' : ''
		);
		printf(
			'<a href="%s" class="nav-tab %s">Лог</a>',
	...
	private function render_settings_tab(): void {
		...
		echo '<p>По умолчанию для новых типов режим выключен. Администраторы и суперадмины могут удалять при любом запрете; действие всё равно пишется в лог.</p>';
		echo '<p>Текущий <code>EMPTY_TRASH_DAYS</code>: <strong>' . esc_html((string) EMPTY_TRASH_DAYS) . '</strong>.</p>';
		...
		echo '<thead><tr>';
		echo '<th>Тип поста</th>';
		echo '<th>Режим</th>';
		echo '<th>Разрешить удаление по EMPTY_TRASH_DAYS</th>';
		echo '</tr></thead><tbody>';
		...
		foreach ($post_types as $post_type => $object) {
			...
			echo '<td>';
			printf(
				'<label><input type="checkbox" name="settings[%s][allow_empty_trash_days]" value="1"%s%s /> Через cron</label>',
				esc_attr($post_type),
	...
	private function render_log_tab(): void {
		...
		echo '<p><label>Статус<br><select name="status">';
		...
		echo '<p><label>Действие<br><select name="log_action">';
		echo '<option value="">Все</option>';
		...
		echo '<p><label>Тип<br><input type="text" name="post_type" value="' . esc_attr($post_type) . '" placeholder="page" /></label></p>';
		echo '<p><button class="button">Фильтр</button></p>';
		echo '</form>';

		echo '<form method="post" style="margin-bottom:16px" onsubmit="return confirm(\'Удалить логи старше 30 дней?\');">';
		wp_nonce_field(self::NONCE_PURGE);
		echo '<input type="hidden" name="delete_guard_action" value="purge_logs" />';
		submit_button('Удалить логи старше 30 дней', 'secondary', 'submit', false);
		echo '</form>';

		echo '<table class="widefat striped">';
		echo '<thead><tr>';
		echo '<th>Дата</th><th>Статус</th><th>Действие</th><th>Пользователь</th><th>Источник</th><th>Пост</th>';
		echo '</tr></thead><tbody>';

		if (!$result['items']) {
			echo '<tr><td colspan="6">Записей нет.</td></tr>';
		}
	...
	private function mode_labels(): array {
		return [
			Settings::MODE_OFF => 'Выключено',
			Settings::MODE_LOG => 'Только логирование',
			Settings::MODE_BLOCK_TRASH => 'Запрет на перемещение в корзину',
			Settings::MODE_BLOCK_DELETE => 'Запрет на полное удаление',
		];
	}
	...
	private function status_labels(): array {
		return [
			Logger::STATUS_ALLOWED => 'Одобрено',
			Logger::STATUS_DENIED => 'Отказ',
		];
	}
	...
	private function action_labels(): array {
		return [
			Logger::ACTION_TRASH => 'В корзину',
			Logger::ACTION_DELETE => 'Полное удаление',
			Logger::ACTION_RESTORE => 'Восстановление',
		];
	}
	...
	private function format_post_link(int $post_id, string $post_type, string $title): string {
		$label = $title !== '' ? $title : '(без названия)';
